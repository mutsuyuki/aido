...
- **成果物記録**: 各ステップの出力を `runs/` に自動保存。開発ログとして後から解析可能
- **canonical state path**: 完了済みフェーズは `work_dir/.aido/state/<phase_id>/` に symlink で公開され、後続フェーズの AI が timestamp 非依存の安定パスから読み込める。プロンプト埋め込みの truncate なし
...
- **前フェーズ成果物への path-addressable アクセス**: 完了済みフェーズは `work_dir/.aido/state/<phase_id>/` に symlink で公開される。後続フェーズのプロンプトには `## 利用可能な前フェーズ成果物` としてファイル一覧（パスとサイズ）だけが注入され、AI は必要なファイルだけを安定パスから読みに行ける（プロンプト埋め込みの truncate なし）
