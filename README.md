...
# 2. 仕様を記述
#    workspace/settings/my-app/context/app_spec.md を編集
#    workspace/settings/my-app/project.yaml の name, work_dir, checks を編集
...
python main.py run workspace/settings/my-app/improve.yaml --auto-approve \
  --input "気の利いた機能を1つ追加して"
python main.py run workspace/settings/my-app/fix.yaml --auto-approve \
...
└── workspace/                   # gitignored（ランタイム用）
    ├── projects/                # 生成されたアプリ（各自独立git repo可）
    │   └── my-app/
...
project:
  name: "My App"
  work_dir: "../../projects/my-app"  # このファイルからの相対パス
...
パイプライン実行時、各ステップの出力が `workspace/runs/<project>/<timestamp>/` に自動保存される。

```
workspace/runs/my-app/20260325_143020/
├── phase_01/
│   └── attempt_01/
...
# プロジェクト固有環境で起動（workspace/docker/<project>/Dockerfile を使用）
./DockerRun.sh gemma-chat
./DockerRun.sh news-feeling
```

共通の `Dockerfile` には Python, Node.js, LLM CLI ツール等が含まれる。Flutter SDK 等のプロジェクト固有の依存は `workspace/docker/<project>/Dockerfile` で `FROM` 継承して追加する。
