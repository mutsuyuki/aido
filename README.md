...
- **成果物記録**: 各ステップの出力を `.aido/runs/` に自動保存。完了済みフェーズは `.aido/state/` に symlink で公開され、後続フェーズから安定パスで参照可能
...
パイプライン実行時、各ステップの出力が `workspace/<project>/.aido/runs/<timestamp>/` に自動保存される。

```
workspace/my-app/.aido/runs/20260325_143020/
├── phase_01/
│   └── attempt_01/
...
完了済みフェーズは `.aido/state/<phase_id>/` に symlink で公開され、後続フェーズの AI が安定パスから参照できる。`pipeline_summary.json` や `log.json` から成否・失敗原因の事後分析も可能。
...
--resume-run .aido/runs/20260325_185807    # 前回の成果物を引き継ぐ
```
